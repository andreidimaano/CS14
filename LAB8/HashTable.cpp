...
*  used for inde`x into the array in hash table
...
void HashTable::put(const string &s, int score) {
	list<WordEntry>* wordList = (hashTable + computeHash(s));
	 if(contains(s)){
		for(list<WordEntry>::iterator i = wordList->begin(); i != wordList->end(); ++i){
			if(i->getWord() == s){
				i->addNewAppearance(score);
			}
		}
	 } else {
		wordList->push_back(WordEntry(s,score)); 
	 }
}
